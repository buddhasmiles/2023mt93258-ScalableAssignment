...
namespace Services
{
    public class UserPaymentService : IUserPaymentService
    {
        ...
        /// <returns></returns>
        public IdentityDto CreatePaymentDetail(Guid userId, PaymentCreateDto newPayment)
        {
            ...
            if (userPaymentRepository.IsPaymentExists(userId, newPayment.Name, newPayment.PaymentValue))
            {
            ...
            userPaymentRepository.AddPayment(customerPayment);
            ...
            return new IdentityDto { Id = customerPayment.Id };
        }
        ...
        public void UpdatePaymentByPaymentId(Guid userId, Guid paymentId, PaymentUpdateDto updatedPayment)
        {
            ...
            {
                paymentDetailExists = userPaymentRepository.IsPaymentExists(userId, name: updatedPayment.Name);
                propertyExists = "Name";
            }

            if (updatedPayment.PaymentValue != null && !paymentDetailExists)
            {
                paymentDetailExists = userPaymentRepository.IsPaymentExists(userId, paymentValue: updatedPayment.PaymentValue);
